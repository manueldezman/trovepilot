...
const ADJUST_TROVE_TYPEHASH = keccak256(
  stringToBytes(
...
export function computeAdjustTroveDigest(args: {
  ...
  // Must match Mezo BorrowerOperationsSignatures._verifySignature usage:
  // structHash = keccak256(abi.encodePacked(typeHash, abi.encode(fields...), nonce, deadline))
  const data = encodeAbiParameters(
    ...
    [args.collWithdrawal, args.debtChange, args.isDebtIncrease, args.assetAmount, args.borrower, args.recipient]
  );

  const structHash = keccak256(concatHex([ADJUST_TROVE_TYPEHASH, data, pad(toHex(args.nonce), { size: 32 }), pad(toHex(args.deadline), { size: 32 })]));
